...
template <typename t>
void generate_const_array(t arr[], int s, int _max){
    std::uniform_int_distribution<int> rng(0, _max);
    t const_value = get_random_value(rng,arr);
...
void bubble_sort(t* arr, int s){
    if (s > 10000){
        cout << " dimensiunea prea mare pt bubble sort (ar dura prea mult)\n";
        return;
    }
    for (int i = 0; i < s; i++){
        for (int j = 0; j < s-i; j++){
    ...
    for (int i = 0; i < t; i++){
        ...
        for (int i = 1; i <= generate_nr; i++){
            ...
            case 3:
                ...
                break;
            case 4:
                generate_const_array(v.data,v._size,v._max);
